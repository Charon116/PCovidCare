...
function createScheduleByDoctor(scheduleArr) {
    $("#createNewScheduleDoctor").on("click", function () {
        if (scheduleArr.length === 0) {
            alertify.error('Chưa chọn giờ làm việc để lưu');
            return
        }
...
function handleChangeDatePicker(currentDate) {
    $('#datepicker').datepicker().on('changeDate', function (event) {
        ...
        if (dateConvert >= currentDateConvert) {
            ...
            alertify.error('Không thể tạo giờ làm việc trong quá khứ');
        }
    });
}
        ...
        success: function (data) {
            ...
            data.object.newPatients.forEach((patient) => {
                ...
                <tr>
                    ...
                    <button  type="button" data-patient-id="${patient.id}" class="ml-3 btn btn-danger btn-new-patient-cancel">Huỷ</button>
                    </td>
                </tr>
                `;
            });
            ...
            data.object.pendingPatients.forEach((patient) => {
                ...
                <tr>
                    ...
                    <button  type="button" data-patient-id="${patient.id}" class="ml-3 btn btn-danger btn-pending-patient-cancel"> Huỷ </button>
                    </td>
                </tr>
                `;
            });
...
function handleAfterCallingPatient() {
    $('#btn-confirm-patient-done').on('click', function (e) {
        if (!confirm('Bạn đã gọi điện để xác nhận xem bệnh nhân có đặt lịch hẹn hay không chưa ?')) {
            return;
        }
        let countPending = +$('#count-need').text();
...
function handleCancelBtn() {
    $('#btnCancelBookingPatient').on('click', function (e) {
        ...
        if (data.reasonCancel === 'reason3') {
            ...
            text = 'Bệnh nhân đã hủy lịch hẹn';
        } else {
            text = 'Cuộc hẹn phòng khám bị spam, không có thật'
        }
    ...
    $('#btnSendFilesForms').on("click", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                alertify.success('Gửi hồ sơ khám thành công');
            },
            error: function (error) {
                alertify.error('Đã xảy ra lỗi, vui lòng thử lại sau!');
                console.log(error);
